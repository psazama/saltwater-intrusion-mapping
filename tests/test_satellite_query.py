...
def create_mock_geotiff(path: Path, bounds: list, width: int = 100, height: int = 100, bands: int = 3):
...
def test_download_gee_multiband_small_image():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with patch('swmaps.core.satellite_query.initialize_ee'), \
             patch('swmaps.core.satellite_query.get_mission') as mock_get_mission, \
             patch('swmaps.core.satellite_query.requests.get') as mock_requests, \
...
def test_download_gee_multiband_large_image_triggers_tiling():
    ...
    from swmaps.core.satellite_query import download_gee_multiband, tile_and_download_gee_image
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Mock GEE components
        mock_image = MagicMock()
        mock_image.get = MagicMock(return_value=MagicMock(getInfo=lambda: "large_test_id"))
        mock_image.select = MagicMock(return_value=mock_image)
        mock_image.reproject = MagicMock(return_value=mock_image)
        mock_image.clip = MagicMock(return_value=mock_image)
        ...
        with patch('swmaps.core.satellite_query.initialize_ee'), \
             patch('swmaps.core.satellite_query.get_mission') as mock_get_mission, \
             patch('swmaps.core.satellite_query.tile_and_download_gee_image') as mock_tile_download, \
             patch('swmaps.core.satellite_query.merge_geotiff_tiles') as mock_merge, \
             patch('swmaps.core.satellite_query.ee.Geometry.BBox') as mock_bbox:
